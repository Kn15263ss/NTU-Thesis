...
def computeAUROC(dataGT, dataPRED, classCount):
    ...
    datanpGT = np.concatenate(dataGT, axis=0)  # .cpu().numpy()
    datanpPRED = np.concatenate(dataPRED, axis=0)  # .cpu().numpy()

    for i in range(classCount):
        outAUROC.append(roc_auc_score(datanpGT[:, i], datanpPRED[:, i]))
    return outAUROC

def buildLogger(output_path):
    logger = logging.getLogger('')
    logger.setLevel(logging.INFO)
    format = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    fh = logging.handlers.RotatingFileHandler(output_path, maxBytes=(1048576 * 5), backupCount=7, mode='w')
...
class ReduceLROnPlateau(object):
    ...
    def __init__(self, optimizer, logger, mode='min', factor=0.1, patience=10,
    ...
    def _reduce_lr(self, epoch):
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            if old_lr - new_lr > self.eps:
                ...
                if self.verbose:
                    self.logger.info('Epoch [' + str(epoch) + '] reducing learning rate'
                          ' from {:.4e} to {:.4e}.'.format(old_lr, new_lr))
